...
能力。`WRITE_RESTRICTED` 不隔离读取。用户显式配置的 `forbid_read` 仍使用临时
拒绝 ACE，并保留安全描述符快照、崩溃残留标记、恢复和全生命周期串行化；
Reasonix 自己的全局凭据 `.env` 不会自动加入 Windows 的 `forbid_read`，避免
同一用户身份的宿主设置进程被一并拒绝。凭据值仍会从模型控制的子进程环境中
过滤，但 Windows 本地工具可以主动读取当前用户可读的文件。
...
凭据读取先尝试正常文件访问。遇到权限拒绝时，只有匹配的沙箱崩溃残留记录能够证明
所属进程已退出，且没有对应的活跃或无法确认状态的记录，才会移除全局 `.env` 上唯一
一条当前用户的显式 `DENY RX`。修复会获取沙箱路径锁并重新检查 ACL。缺少残留记录、
存在其他当前用户拒绝条目或不支持的 ACE 类型时不会自动修复，权限错误仍会保留。
仅凭相同权限掩码无法确定该规则属于 Reasonix。
修复成功后会消费死进程的残留记录，避免它以后授权删除无关的 ACL 变更。
