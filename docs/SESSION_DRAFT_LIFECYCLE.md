...
Session, Topic, Controller, session lease, or MCP runtime.
...
references, model/effort/mode/approval/quality settings, Goal settings, and MCP
selection. Runtime objects, credentials, preview URLs, pending paste state, and
submission UI flags are not persisted.
...
Saved draft models are strict execution choices. Config-backed aliases may be
canonicalized to the same provider/model identity, but an unavailable model is
reported before Session reservation and never falls back to another model.
Plugin model references are passed to the runtime resolver and fail there when
unavailable. A retry applies the new operation's frozen settings to the same
