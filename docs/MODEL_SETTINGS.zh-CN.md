...
内置 Bash/PowerShell 后台进程归属于逻辑会话。切换模型连接时保留进程、任务 ID、
输出、停止能力及工作区写租约。关闭标签页仍遵守原有的保留后台运行或停止行为；
真正结束会话才取消其进程。

后台 Agent 子任务仍依赖旧运行时，必须等到真正退出才能重建；发出取消请求不代表已经退出。
输入区显示待应用状态、具体阻塞任务、实际模型及脱敏后的连接地址，并提供查看任务、
停止选中的任务和重试应用。任务完成后合并触发一次应用尝试；真实构建失败等待用户重试
或新的配置版本。自动应用成功不会发送之前被拒收的草稿。

“本次使用当前配置发送”只授权一个提交，后端重新校验运行时、已应用和目标版本、全部旧模型
路由、凭据及权限。下一次提交默认使用最新设置。供应商或模型删除、凭据（包括认证请求头）
变化、限制收紧或无法校验时不可沿用旧配置。单纯主机地址变化可以明确确认；URL 认证、
查询参数或路径变化保守地视为无法确认。界面不显示密钥、URL 认证、查询参数或路径。

确定拒收时保留文字和附件，并解除草稿的提交占用；响应丢失时保持结果未知，先使用原提交 ID
核实结果。已接受回执优先于新的配置检查，避免重复执行。只有真实草稿或历史冲突才显示
“使用已保存版本／保留本窗口版本”，配置问题不会触发这些操作。

Desktop/Serve 协商 `model-application-v1` 能力，旧宿主保持保守行为并隐藏不支持的操作。
远程重连或切换会话后必须重新确认。CLI/ACP 共享后台所有权和模型重建判定，默认使用最新配置。
本次不改变配置／历史格式、模型可见消息或工具 Schema；429 和超长上下文属于另外的问题。

认证条件与配置持久化、runtime 应用分别管理。本地 Ready 只表示具备提交条件，远端仍可能
失败。缺少凭据时，在创建用户消息、执行 hook 或发送 provider 请求之前就会阻止该轮。
401 会阻止同一连接和凭据的后续自动请求；403 会尽量只阻止失败模型。恢复提示会保留草稿
和附件，并提供配置凭据、选择模型、测试连接和一次显式重试。保存或测试成功都不会自动
重放失败任务。
...
进行中的记录位于 Reasonix home 下的私有事务目录。只有旧配置 revision 仍可确认时，恢复
流程才会删除记录明确列出的本次新槽位。发布前先记录最终配置内容的 revision，恢复时必须
同时匹配该 revision 与凭据引用才能补齐回执；不分配凭据槽位的编辑也遵循此规则。
遇到并发修改、路径不可读或清理失败时会保留证据供诊断，不覆盖当前文件。

请求摘要使用私有、持久化的认证密钥，同一请求在进程重启后仍可识别。旧版本未使用稳定
摘要的回执无法验证重放请求的内容，应重新读取设置，不能据此发起一次新的写入。
