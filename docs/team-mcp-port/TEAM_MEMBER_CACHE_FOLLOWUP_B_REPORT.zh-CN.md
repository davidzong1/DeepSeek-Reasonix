...
| 折叠规则 | `internal/provider/anthropic/stream_usage.go` sha256 写入每臂 banner |
...
**并发确实发生了**（独立核验，非假设）：逐记录时间间隔在并发臂上出现**小于单请求时延的间隔**——S1a（串行）间隔稳定在 1.47–2.08s；S3-c2 出现 0.07–0.28s 的间隔；S3-c3 间隔进一步压缩到 0.09–0.93s。挂钟时间也从 67.6s 降到 50.0s 再降到 23.0s，与 1/2/3 的并发档单调一致。
...
**观测**：S1c-large（ctx 592K）与 S4-768k（ctx 792K）的 **`prefix_change_reasons` 全部为空**，`stable_prefix_changed` 未置位，ctx 逐轮单调 +23 无回落。**没有任何一个臂触发 fold。**
...
**首轮**用回退值 0.25 估算 → S4 的 3,840,000 chars 估算 **960,000 > 800,000**，看似越过触发线；但 fold 仍**未触发**，说明首轮的准备路径没有以该估算值进入折叠分支（估算在请求前由 `Prepare` 计算，而 `foldContext` 还受 `planCompaction` 的 pinned-prefix / `minCompactMessages` 约束）。**次轮起**估算改用实测比值 0.2066 → 3,840,000 × 0.2066 ≈ **793,000 < 800,000**，**确定性地低于触发线**。

**判定**：在本任务族（单个巨大首轮 + 逐轮 +23 token）与 1M 窗口下，**fold 边界不可达**。这**不是**"未观察到差异"，而是**没有样本**。
