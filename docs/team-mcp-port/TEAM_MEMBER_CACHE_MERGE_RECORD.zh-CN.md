...
> 状态：**本轮 P4 与兜底证据审计完成；主线整体未闭环。** B 后续 strata 数据因 route/account 偏离预注册条件，只能作为探索性基线；本文记录的是当前工作树审查结果，不代表已提交或发布。
...
> **兜底复核补充（2026-09-25）**：B 后续六臂归档虽有 204 条可审计记录，但实际池条目/AgentUserRef 为 `strata-gw`、route 为 `anthropic/f119dfdb4214`；预注册冻结的是 `deepseek-v4-flash-roojin`、`anthropic/bfcb0811b1c8`。因此正式 Team 分层门禁未满足，记录仅作探索性基线。详见 `TEAM_MEMBER_CACHE_FOLLOWUP_CLOSEOUT.zh-CN.md` §2.6。
...
| 夹具解析冻结 | `internal/cachelab/usage.go` md5 `cc0cda4fcbd22bd7f68602d376656865`（C 的 K3 落地版本；与 B 归档 `cachelab-usage.go.snapshot` 一致）。当前工作树摘要为 `01a5cb37e30cf272a095fce1a09837f1`，差异经复核**仅为字段注释**，去注释后与冻结快照逐字节相同（见 §9.8，已关闭） |
...
> **复算口径差异已解释（2026-09-25 关闭，依据 C `FOLLOWUP_C_AUDIT` §2.2 并经负责人独立复算）**：两个数字是**同一批行的两个口径**——"模型前缀匹配的全部行"与"其中带 `prompt` 键的行"。差额正好是**没有 usage 计数器的行数**（09-23 9 行、09-24 1 行；这些行的键集无 `prompt`/`cache_hit`/`cache_miss`），**不贡献任何 token，因此两种口径的加权率完全相同**。
>
> | 口径 | 09-23 | 09-24 | 共同小时 09-23 | 共同小时 09-24 |
> |---|---:|---:|---:|---:|
> | 前缀匹配全部行 | 1,973 | 1,714 | 174 | 633 |
> | 其中带 `prompt` 键 | 1,964 | 1,713 | 169 | 633 |
> | 加权率（两口径相同） | 83.83% | 67.70% | 86.14% | 76.51% |
>
> **两个数字都正确**，因此**不撤回任何精确数字**；引用时必须注明口径。1,169/1,964 这一行数分解只在"带 `prompt` 键"口径下成立，已据此改标。
...
**判定：C-5 成立。** `--first 1714` 对应 09-23 的大部分全天记录（其中含大量晚间样本）与 09-24 的早间记录，两个样本窗口不匹配。**−16.8pp 撤回为不可比。** §3.1 的口径差异已解释（见上方表），因此共同小时的行数引用**须注明口径**（前缀匹配 174 / 带 `prompt` 键 169）。
...
4. **B 正式分层实验**：后续六臂有 204 条可复现数据，但 route/account 与预注册冻结条件漂移，正式门禁未满足；数据仅为探索性基线。需在冻结 route/account 上重跑，或建立并冻结新的预注册版本后重跑。
...
8. ~~**冻结快照对应关系**~~ —— **已关闭（2026-09-25，依据 C `FOLLOWUP_C_AUDIT` §1 并经负责人独立复算）**：归档快照 `cc0cda4f…` 与当前 `usage.go` `01a5cb37…` 的差异**只有一处**，是 `ReportedUsage.Oracle*` 的字段注释（9 行 → 3 行，为过 repolint 的 `capFieldDoc = 3` 限额而压缩）。**去掉注释行与空行后两份文件逐字节相同**（两份均无块注释、无反引号字符串，故去注释比较可靠）；代码行**零改动**。**当前文件是冻结版本的语义等价物**，K3 测试与 journal 重放已在当前版本上重跑通过。
...
**阶段收口**：P4 与兜底证据审计、历史口径调和、A/C/K1/K3 收尾和本轮 go/no-go 已完成；主线保持开放，直到严格满足预注册条件的 Team 分层、未决根因研究与发布收尾完成。缓存行为优化候选全部 NO-GO；此阶段关闭不代表整项研究或发布完成。
