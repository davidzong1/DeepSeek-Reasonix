...
| R1.2 | `memberEvent` 打标 sink + `teamBackends` 注册表 | R1.1 | **已完成**（发送语义已被 v0.10 的 `memberEventPump` 取代，见 §10） | `internal/cli/team_backends.go`：`memberEvent{member, ev}` + `memberSink`（原为阻塞发送，与主路径 `eventSink` 同语义，靠 1024 缓冲而非丢弃策略——共享通道因此把每个成员的 agent loop 绑在窗口速率上，v0.10 改为每成员有界队列）；`teamBackends` 懒建/复用/LRU 上限 4/`release`/`releaseTeam`/`closeAll`，assembly 失败不留痕以便重试。测试 5 个（懒建幂等、团队参与身份、失败不注册、LRU 只淘汰非当前成员、三条 release 路径各关一次） |
...
| 版本 | 日期 | 变更 | 责任 |
|---|---|---|---|
| v0.13 | 2026-09-22 | **改窗冻帧收尾（P1-1 剩余）+ 成员用量快照离开帧路径（P1-2）**。**P1-1 剩余**：v0.12 之后仍有一处 O(历史) 冻帧——bundle 已装好后再改终端宽度，`reflowTranscript` 会在 Update goroutine 上把整段成员历史重新 markdown 渲染一遍。现在 replay block 的 reflow 走**同一个有界窗口**：`transcriptSource` 新增 `windowed` 标记（只有成员绑定/改窗这条有界链路置位），reflow 命中它时只渲染最新 `replayInlineMessages` 条（`renderReplayBundleWindow`），其余由 `reflowReplayBundle()` **重新起一次后台全量渲染**（cmd 并入 `Update` 已有的 `if widthChanged` 分支，不新增分支——`chat_tui.go` 的 complexity ratchet 零增长；安装发生在后续一条消息上，彼时 wrap 缓存已在新宽度，因此后台算出的 wrap 正是该帧本该自己重建的那份）。**关键正确性**：窗口化 paint 里 `rendered` 只含窗口，但 block 的 transcript source 经新增的 `replayPaint.sourceHistory` 仍保存**整段历史**——否则绑定后立刻改窗会把尾部窗口固化成成员的全部历史（新增 `TestMemberReplayResizeStillRendersTheWholeHistory` 钉住这条）。**同一处差点踩到的回归**：`replayActiveBranch`（`/resume`、`/rewind`、`/branch`、启动期恢复会话）也提交 `transcriptSourceReplayBundle`，一开始被一并窗口化——那会让恢复会话只显示最新 24 条。因此窗口是有标记的：未置位者（聊天会话自己的重放）提交与 reflow 都按**全量**渲染，并新增 `TestSessionReplayBlockStaysWhole` 作为该回归的守卫。**改窗不钉尾**：`replayLoad` 新增 `pin`（绑定置位、改窗不置位），安装时 `forceGotoBottom = load.pin`——用户可能已向上滚进窗口读历史，改窗不该把人拽回底部（`TestMemberReplayPinsTheTailOnABindButNotAResize`）。`clearTranscriptDisplay` 经 `replayWindow.reset()` 一并清 block/load（generation 继续递增），飞行中的全量渲染不会装进下一个显示恰好落在该下标的 block。**P1-2**：伙伴（follower）的用量文档原本由状态栏**每帧按 1s 节流读盘**（`usageSnapshot`，一次 stat+parse）。新增 `refreshUsage`（在锁外做 I/O、锁内安装）+ `chatTUI.refreshBoundMemberUsage()`，由既有 roster tick 每秒把绑定成员的用量交后台读一次；`usageSnapshot` 在「已被 tick 刷新过」之后**只服务内存**（`usageTicked` 标记），TTL 仍由文档自身时间戳决定（写入者停摆照旧隐藏）。无 tick 的宿主（测试、非团队窗口）行为与旧实现完全一致，既有 `TestFollowerUsageReadIsThrottled` 不改。**测试**：`team_replay_test.go` 新增 4 个（改窗走窗口，且**渲染结果确实由该帧自己的 cmd 交付**——从 `Update` 返回的 cmd 里取回 paint，而非从窗口状态重算，否则「算了却没交回循环」这类接线缺失会静默通过；窗口化后仍保留整段历史；绑定钉尾/改窗不钉尾；未窗口化会话重放保持完整）、`team_follower_usage_test.go` 新增 2 个（tick 刷新后帧路径**零读盘**且仍渲染写入者数字；刷新不改变「写入者停摆即隐藏」）；为让 `Update` 真帧路径可跑，`stubBackend` 补 `AutoApproveTools`/`InboxSnapshot`（此前是内嵌空接口的 promoted 调用，一渲染即 panic）。**验证**：gofmt clean；`go build ./...`；`go vet ./internal/cli/`；`go test ./internal/cli/ ./internal/team/...` 全绿（42.8s）；改动路径 `-race -count=2` 绿；全包 `-race` 仅剩 v0.12 已记录的**既有测试内竞态**（`TestEscalationWakeNamesTheQueue`、`TestTerminalSizeRecoveryFollowsTheTerminalUnderTheFrame`，栈内无本轮文件；另 `TestTeamHubLeaderAndMemberDoNotSerialize` 偶发——该用例靠 40ms sleep 测并发，负载高时两个 goroutine 不同时在途，隔离跑 10/10 绿，与本轮无关）；repolint 本轮**文件级零新增**、complexity 归位（本轮曾一度因 `chat_tui_stream.go` 行数 +4 与该文件注释超限被点出，已分别用 `replayWindow.reset()` 与注释收敛解掉）；剩余两条红项同源，均来自他人进行中的 `internal/provider/anthropic/messages_usage.go`（该文件 4 行注释超限，以及由此把 repo essay 总数顶到 1758 > 1757），未擅自修。**遗留**：每 tick 的 owner 指纹读取（`syncAmbientOwnerUsage`）与 roster store 读取仍在 Update 线程（约 1Hz 小文件，前者两阶段化会牵动跨窗口同步契约，不抵风险）；§8-36 的 Ack 时点未改 | — |
| v0.12 | 2026-09-21 | **成员切换重放/历史重建移出 Update 线程（P1-1）+ 成员 cockpit worker（P0-2 闭环）**。**P1-1**：新增 `internal/cli/team_replay.go`，把 replay bundle 的渲染与换行抽成纯函数（`renderReplayBundleBlock`，原方法改为委托，显示路径唯一不漂移）。同步部分**有界**：历史 ≤ `replayInlineMessages`(24) 条时内联渲染，与旧实现逐字节相同；更长时**同步只画最新 24 条窗口**，全量 bundle 由后台 goroutine 渲染**并在同一 contentWidth 上换行**，回投 `teamReplayReadyMsg`（骑 `teamRosterRefreshMsg.replay`，沿用「结果骑 tick，不开第二个 update 分支」的既有约定），安装时 `installWrappedBlock` 把后台算出的 wrap **预填进 wrap 缓存**——于是整段历史的 markdown 渲染与 wrap 都不再出现在 Update goroutine 上（install 只做 O(行数) 的 flatten/克隆）。渲染途中终端改宽：结果按 `msg.width != m.width` 丢弃并**按当前宽度重投**；被后续绑定取代的结果按 `replay.gen` 丢弃（窗口显示的是真实尾部历史，不是占位符，丢弃也不损坏）。**边界**：只有能把 cmd 交回 Update 循环的路径用 `replayBounded`（`switchTeamMember`、跨窗口 `replayBoundHistory`），ambient 归还（`closeSession`→`unbindTeamMember`，无 cmd 可回传）与 `/model` 重建、配额 failover 保持内联 `replayInline`。**主题安全**：后台渲染会读 `activeCLITheme` 与其派生样式，故新增 `themeMaterializeMu`（`withThemeMaterialized` 读锁 / `materializeTheme` 写锁 / `tryMaterializeTheme`），theme sweep 的每帧 palette 交换拿不到写锁就跳过该帧而非等一整次渲染。`batchCmds` 从 `chat_tui.go` 移到 `chat_tui_events.go`，使 `chat_tui.go` 的 file-size/complexity ratchet 零增长。**P0-2**：新增 `internal/cli/team_member_cockpit.go` —— **每成员一个常驻 worker**（`memberCockpit`，生命周期挂在 `teamBackends`，`closeAll` 关闭），把 owner 历史发布（成员身份读取 + 跨进程锁下的 `BumpHistory` 读改写）从事件处理路径移出；同成员命令串行、不同成员并行（`TestCockpitSerializesPerMemberAndRunsMembersInParallel` 钉住 peak=1 且 bump 计数真实）；结果经 roster tick 的 `collectCockpitResults`（**非阻塞**，空则不产生消息）在 Update goroutine 上 `applyCockpitResults` 应用——「TUI 发命令、收快照」的状态改动仍全部在 Update 线程。无 registry 的宿主（测试、非交互）保留内联回退，行为与旧实现一致；新增 `recordMemberOwnerHistoryWith` 让「读到的身份就是发布的身份」。**测试**：`team_replay_test.go` 5 个（内联边界；尾部窗口→全量安装且预填 wrap 与懒重建**逐行一致**；被取代的 paint 丢弃；改宽重投；主题帧不等待后台渲染）+ `team_member_cockpit_test.go` 5 个（发布确实离开调用者、同成员串行/跨成员并行、无身份跳过 + 关闭拒绝新活、idle 跟踪在途、TUI 侧成功清错与失败上报）；`waitForCockpit` 把 4 处既有 owner-history 断言从「与后台写竞态」改成确定性。**验证**：gofmt clean；`go build ./internal/...`；`go vet ./internal/cli/`；`go test ./internal/cli/ ./internal/team/...` 全绿；改动路径 `-race -count=3` 绿；repolint 本轮文件零新增（唯一红项属于他人进行中的 `internal/provider/anthropic/messages_usage.go`）。全包 `-race` 另有两个**与本轮无关的既有测试内竞态**（`TestEscalationWakeNamesTheQueue`：`team_escalation_test.go` 的假后端字段被 escalation worker 并发写；`TestTerminalSizeRecoveryFollowsTheTerminalUnderTheFrame`：`tui_diagnostics_test.go` 的测试局部变量被 watchdog 回调并发写），两者栈内均无本轮改动文件。**遗留**：已安装全量 bundle 后再改窗仍走既有 reflow（全量重渲染+重换行，未后台化）；启动期 resumed session 的首次 banner 重放仍是内联；每 tick 的 owner 指纹读取仍在 Update 线程（约 1Hz，量级小）；§8-36 的 Ack 时点未改 | — |
...
| v0.11 | 2026-09-21 | **inbox 预取：命令注入的 board 读取移出提交路径（P0-3）**：`teamInboxWire` 新增每成员读前批（`prefetched`/`fetching`/`acks`），成员绑定（`bindBackend` 里 owner.Member≠"" 的唯一收口）与每次注入成功后各起一个后台 goroutine 预取下一批，提交路径（Enter 键）命中预取时**零 board 读取**；未命中仍回退历史同步读，语义零改动。**Ack 时点未改**（仍在 `inject` 内、write-before-commit，§8-36 的 at-most-once 问题本轮不动）。**并发正确性**：预取在后台与 ack 竞争，实测重现两类重复注入——①读在 ack 前开始、落库在 ack 后；②读与落库都在 ack 前但消费在 ack 后。修法：批上盖 `ackEpoch`（落库时的 ack 计数），`takePrefetched` 在消费时校验，不等即丢弃并回退同步读；`resetInboxes` 一并清 `inboxes`/`prefetched`/`acks`，旧 generation 的批不会骑上新窗口；`inboxFor` 的 map 改受同一把锁（现在有后台调用者）。测试新增 4 个：`chat_tui_team_inbox_prefetch_test.go`（预取服务提交路径且被消费掉、reset 丢弃旧批、未绑定不建批、**被 ack 越过的批不得再注入**的确定性回归）。验证：`go test ./internal/cli/ -run 'TeamInbox|TestTeamTurnInjectsInboxAtSubmit' -count=20 -race` 绿（修复前 `-count=8 -race` 可稳定重现失败）；全包 42.9s 绿；repolint clean（1157 baselined） | — |
| v0.10 | 2026-09-21 | **成员事件通道去耦（成员数一多就卡的第一根因）**：`memberSink` 的阻塞式共享通道（`chan memberEvent`，cap 1024）替换为 `memberEventPump` —— 每成员有界队列 + 全局到达序（`seq`，与旧共享通道的投递顺序同构，切换不改可观测顺序）+ delta 合并（Text/Reasoning/ToolProgress）+ 溢出先淘汰最旧 delta、再淘汰最旧非终态事件，终态（TurnStarted/Message/TurnDone/ApprovalRequest/AskRequest）永不丢。**动机**：共享通道把每个成员的 run goroutine 绑在窗口 update 速率上，TUI 一忙就填满 1024 缓冲，于是**所有**成员一起被堵——正是"成员多起来变卡"的机制。现在 Emit 只入队，单个成员只可能被自己的积压拖慢。新增 `internal/cli/team_event_pump.go`（pump/合并/淘汰/公平性）+ `team_event_pump_test.go` 4 测；`waitForMemberEvent` 改消费 pump，关闭时返回 nil cmd，不再把幽灵零事件（Kind 零值=TurnStarted）灌进绑定成员的 transcript。门禁：gofmt clean、`go test ./internal/cli/` 全绿（42.5s）、`-race` 团队子集绿（24.0s）、repolint clean（1157 baselined，baseline 未加宽）。**遗留**：P0-2 每成员 supervisor goroutine、P0-3 inbox Fetch/Ack 移出 UI 线程、P1 主线历史重建/切换重放后台化未做 | — |
