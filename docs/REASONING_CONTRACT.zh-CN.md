...
同一份解析出的词汇表在所有可能携带力度的边界上给出一致结论：在任一边界被拒绝的档位，
在其他边界同样被拒绝，且没有任何边界会把它改写成相邻档位。
