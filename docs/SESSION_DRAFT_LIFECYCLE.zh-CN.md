...
Reasonix Desktop 将本地手动新建的会话作为编辑草稿，直到第一次执行被持久化接收。打开草稿不会创建 Session、Topic、Controller、会话租约或 MCP 运行时。
...
持久化内容包括输入文字、结构化 invocation、附件与工作区引用、粘贴块、选中文本引用、历史 Session 引用、模型、effort、模式、审批、质量下限、Goal 及 MCP 选择。Controller、凭据、预览 URL、待处理粘贴和提交中 UI 状态不写入草稿库。
...
保存进草稿的模型是严格执行选择。配置模型允许规范化为同一 provider/model 身份，但模型失效会在预留 Session 前报错，绝不回退到另一模型。插件模型引用交由运行时解析，不可用时在原引用上失败。重试会把新操作冻结的设置应用到同一个预留 Session 身份。
