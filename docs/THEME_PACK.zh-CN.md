...
- 同时支持 Workbench / Creation，以及 `auto` / `light` / `dark`
- **不包含**在线主题市场、云同步或脚本插件

## 主题体验（设置信息架构）

聊天代码块使用语言标题栏和常显复制按钮，与代码正文共用不透明背景。
关键字、字符串、函数、数字和注释分别着色，并根据最终代码及 Diff 背景
校正至至少 4.5:1 对比度，覆盖反转明暗和半透明自定义配色。壁纸透明度
不影响代码及标题栏按钮；切换主题通过 CSS 为已有语法节点重新着色。
正在生成的代码块使用同一查看器，未变化的高亮行保留 DOM；较长代码追加时，
在空闲高亮完成前保留已有前缀颜色。不支持的语言继续显示纯文本。

在 `desktop/frontend` 运行 `pnpm test:code-browser`，验证实际 Markdown
组件的主题、流式生成、复制和窄窗口行为。
