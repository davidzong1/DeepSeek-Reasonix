...
func build(ctx context.Context, opts Options) (*BuildResult, error) {
	...
	if opts.RequireKey && opts.ProviderResolver == nil && providerext.PluginRefOwner(modelName) == "" {
		if err := cfg.Validate(modelName); err != nil {
			if entry.RequiresAPIKey() && entry.APIKey() == "" {
				// One verdict, two audiences: the diagnostics UI reads the typed
				// state, while the headless caller needs the strict observability
				// text. Carry that text as Message so both hold on one error.
				authentication.Message = fmt.Errorf("%w (config: %s)",
					strictEntryFailure(entry, modelName, err), providerConfigSource(cfg)).Error()
	...
	if canReuseSkills {
		...
		skillStore = skill.New(skill.Options{
			ProjectRoot: root, TeamSkillsRoot: teamSkillsRoot, TeamRole: opts.TeamRole, CustomPaths: cfg.SkillCustomPaths(), PluginPaths: cfg.PluginPackageSkillOwners(),
		...
		allSkillStore = skill.New(skill.Options{ProjectRoot: root, TeamSkillsRoot: teamSkillsRoot, TeamRole: opts.TeamRole, CustomPaths: cfg.SkillCustomPaths(), PluginPaths: cfg.PluginPackageSkillOwners(), PluginAgentPaths: cfg.PluginPackageAgentOwners(), ExcludedPaths: cfg.SkillExcludedPaths(), MaxDepth: cfg.SkillMaxDepth(), Stderr: io.Discard, Watch: watchSkills, WatchService: skillWatchService})
	...
	newTaskTool := func() *agent.TaskTool {
		return agent.NewTaskToolWithOptions(agent.TaskToolOptions{
			...
			CacheAwareCompaction: cfg.Agent.CacheAwareCompaction, DisableLowYieldLatch: !cfg.LowYieldLatchEnabled(), DisableShapeDiagnosis: !cfg.ShapeDiagnosisEnabled(),
