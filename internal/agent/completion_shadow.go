...
// summary: counts, enums, and gap kinds, never a path or a command. The team
// orchestration contract publishes it even though upstream's quality gate no
// longer consumes the channel.
func completionReportAudit(rep completion.Report) event.CompletionReportAudit {
	audit := event.CompletionReportAudit{
		...
		Verifications:  len(rep.Verifications),
	}
	for _, gap := range rep.Gaps {
