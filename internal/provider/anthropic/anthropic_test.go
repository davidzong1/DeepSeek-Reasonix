...
func TestBuildRequestScopesLegacyTupleMigrationToMiMo(t *testing.T) {
	...
	mimo := (&client{endpoint: endpointDialect{mimo: true}}).buildRequest(context.Background(), req)
...
func TestNewDetectsMiMoSchemaDialect(t *testing.T) {
	for _, tc := range []struct {
		...
		if got := p.(*client).endpoint.mimo; got != tc.want {
			t.Errorf("New(%q).endpoint.mimo = %v, want %v", tc.baseURL, got, tc.want)
		}
	}
}
...
func TestNewScopesNativeCacheWritePricingToAnthropic(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := p.(*client).endpoint.native; got != tc.want {
...
data: {"type":"message_stop"}
`
	c := &client{name: "anthropic", endpoint: endpointDialect{native: true}}
	resp := &http.Response{Body: io.NopCloser(strings.NewReader(sse))}
	ch := make(chan provider.Chunk)
	go c.readStream(context.Background(), resp, ch)
