...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 11", "[desktop]", `currency = "CNY"`, "[billing]", `display_currency = "CNY"`, `theme = "dark"`, `terminal_theme = "auto"`, `close_behavior = "background"`, `status_bar_style = "text"`, `default_tool_approval_mode = "workspace-write"`, `check_updates = false`, "[notifications]", "[tools.shell]"} {
