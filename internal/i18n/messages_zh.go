...
var Chinese = Messages{
	...
	ImageRequestRecovery:             "这张图片未能发送给模型。请重新添加图片后发送；如果仍然失败，请检查图片模型设置。聊天记录已保留",
	ExtensionRequestRecovery:         "扩展未能提供可用的模型请求。请在设置中检查最近启用的扩展后重试，聊天记录已保留",
	ExtensionBlockRecovery:           "请根据扩展给出的原因检查其设置后重试。重复发送同一条消息不能解除此拦截。",
	ContextLimitRecovery:             "自动整理上下文后，请求仍然过长。请缩短最新消息，或切换到支持更长上下文的模型。原始聊天记录已保留",
	SessionSaveRecovery:              "聊天记录暂时无法保存。请保留当前会话，并尽可能导出备份；检查磁盘剩余空间和目录写入权限后再继续",
	...
	PromptTruncatedByServerNotice:    "服务端接受了该请求，但静默丢弃了部分提示词，模型没有收到完整的指令或工具定义。请调大服务端自身的上下文长度（Ollama：OLLAMA_CONTEXT_LENGTH 或 Modelfile 的 num_ctx），再把 context_window 设为相同的值。",
	...
	ChatStatusIdle:                         "就绪",
	ChatStatusCycleHint:                    "Shift+Tab 循环仅可查看/工作区内修改/YOLO/计划 · Ctrl+Y YOLO",
	ChatStatusCycleHintCompact:             "Shift+Tab 仅可查看/工作区内修改/YOLO/计划 · Ctrl+Y YOLO",
	...
	PlanModeBashTrustReason:                "这条 bash 命令不在 Reasonix 内置只读集合中。只有在确认这个精确前缀用于计划和研究时是只读的，才应批准。权限预设不能代替这次信任确认。",
	...
	CompactionAuto:             "自动",
	CompactionManual:           "手动",
	CompactionStopping:         "正在停止压缩…",
	CompactionSaving:           "正在保存压缩结果…",
	CompactionNoHistory:        "暂无可压缩的历史",
	CompactionStopped:          "已停止压缩",
	CompactionStoppedPartial:   "已停止压缩；部分上下文已压缩",
	...
	WorkModeUsage:                "用法：/preset <旧模式值>（已退役；已知值统一使用标准执行）",
	WorkModeDeprecatedNotice:     "执行模式已退役。已知旧值仅为兼容而接受，并统一使用标准执行。",
	QualityFloorApplied:          "执行模式已退役；当前继续使用标准执行。",
	RewindNone:                   "暂无可回滚的内容",
	...
	ProviderErrAuthRejected:         "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题。请稍后再试，或检查 .env 中的密钥 / 运行 `reasonix setup`。",
	...
	ProviderErrRateLimited:          "请求速率达到上限 (HTTP 429)：请求过于频繁 (TPM/RPM)。请放慢速率或稍后再试。",
	ProviderErrServer:               "服务器故障 (HTTP 500)：服务端内部错误。请稍后再试。",
	ProviderErrServerBusy:           "服务器繁忙 (HTTP 503)：服务端负载过高。请稍后再试。",
