...
var ChineseTraditional = Messages{
	...
	ImageRequestRecovery:             "這張圖片未能傳送給模型。請重新加入圖片後傳送；如果仍然失敗，請檢查圖片模型設定。聊天記錄已保留",
	ExtensionRequestRecovery:         "擴充功能未能提供可用的模型請求。請在設定中檢查最近啟用的擴充功能後重試，聊天記錄已保留",
	ExtensionBlockRecovery:           "請根據擴充功能給出的原因檢查其設定後重試。重複傳送同一則訊息不能解除此攔截。",
	ContextLimitRecovery:             "自動整理上下文後，請求仍然過長。請縮短最新訊息，或切換到支援更長上下文的模型。原始聊天記錄已保留",
	SessionSaveRecovery:              "聊天記錄暫時無法儲存。請保留目前會話，並盡可能匯出備份；檢查磁碟剩餘空間和目錄寫入權限後再繼續",
	...
	ChatStatusCycleHintCompact:             "Shift+Tab 僅可查看/工作區內修改/YOLO/計畫 · Ctrl+Y YOLO",
	...
	PlanModeBashTrustReason:                "這條 bash 命令不在 Reasonix 內建唯讀集合中。只有在確認這個精確前綴用於計劃和研究時是唯讀的，才應核准。權限預設不能代替這次信任確認。",
	...
	CompactionAuto:             "自動",
	CompactionManual:           "手動",
	CompactionStopping:         "正在停止壓縮…",
	CompactionSaving:           "正在儲存壓縮結果…",
	CompactionNoHistory:        "暫無可壓縮的歷史",
	CompactionStopped:          "已停止壓縮",
	CompactionStoppedPartial:   "已停止壓縮；部分上下文已壓縮",
	...
	WorkModeUsage:                "用法：/preset <舊模式值>（已退役；已知值統一使用標準執行）",
	WorkModeDeprecatedNotice:     "執行模式已退役。已知舊值僅為相容而接受，並統一使用標準執行。",
	QualityFloorApplied:          "執行模式已退役；目前繼續使用標準執行。",
	RewindNone:                   "暫無可回滾的內容",
	...
	ProviderErrRateLimited:          "請求速率達到上限 (HTTP 429)：請求過於頻繁 (TPM/RPM)。請放慢速率或稍後再試。",
	ProviderErrServer:               "伺服器故障 (HTTP 500)：服務端內部錯誤。請稍後再試。",
	ProviderErrServerBusy:           "伺服器繁忙 (HTTP 503)：服務端負載過高。請稍後再試。",
...
`,
	...
	ProviderErrAuthRejected:    "認證失敗 (HTTP 401)：服務端拒絕了你的 API key。可能是 key 錯誤或已過期，也可能是服務端出現瞬時鑑權/額度問題。請稍後再試，或檢查 .env 中的金鑰 / 執行 `reasonix setup`。",
