...
var English = Messages{
	...
	ImageRequestRecovery:             "This image could not be included. Attach it again and resend; if it still fails, check the image model settings. Your conversation is kept",
	ExtensionRequestRecovery:         "An extension could not prepare a usable model request. Review the recently enabled extensions in settings and retry; your conversation is kept",
	ExtensionBlockRecovery:           "Review the extension's reason and settings before retrying. Repeatedly resending the same message will not resolve this block.",
	ContextLimitRecovery:             "Automatic context recovery could not fit this request. Shorten the latest message or choose a model with a larger context window. The original history is kept",
	SessionSaveRecovery:              "The conversation could not be saved. Keep it open and export a backup if available. Check free disk space and folder permissions before resuming",
	...
	ChatStatusCycleHintCompact:             "Shift+Tab read-only/workspace/YOLO/plan · Ctrl+Y YOLO",
	...
	PlanModeBashTrustReason:                "This bash command is not in Reasonix's built-in read-only set. Confirm only if this exact prefix is read-only for planning and research. Permission presets cannot answer this trust prompt.",
	...
	CompactionStopping:         "stopping compaction…",
	CompactionSaving:           "saving compaction result…",
	CompactionNoHistory:        "No history to compact",
	CompactionStopped:          "Compaction stopped",
	CompactionStoppedPartial:   "Compaction stopped; part of the context was compacted",
	...
	WorkModeUsage:                "usage: /preset <legacy-value> (retired; recognized values use standard execution)",
	WorkModeDeprecatedNotice:     "Execution modes have been retired. Recognized legacy values are accepted for compatibility and use standard execution.",
	QualityFloorApplied:          "Execution modes have been retired; standard execution remains active.",
	...
	ProviderErrAuthRejected:         "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue. Try again shortly, or check the key in .env / run `reasonix setup`.",
	...
	ProviderErrRateLimited:          "Rate limit reached (HTTP 429): too many requests (TPM/RPM). Slow down or try again shortly.",
	ProviderErrServer:               "Server error (HTTP 500): the provider hit an internal fault. Please try again later.",
	ProviderErrServerBusy:           "Server busy (HTTP 503): the provider is overloaded. Please try again shortly.",
