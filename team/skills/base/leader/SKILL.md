...
3. Split the request into subtasks that can each be delivered and verified
   ...
   one member. Files: `atomic_read`/`atomic_write`, not `cat`/`sed`/`>>`. Insert beside a line by `patch`ing that single line with `range`, keeping the original line in `content`.
